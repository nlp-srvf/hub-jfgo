...
class DataGenerator:
    ...
    def load(self):
        ...
        with open(self.path, encoding="utf8") as f:
            ...
            for segment in segments:
                ...
                #使用bert词表，自动切割为单字去匹配编码
                input_ids = self.encode_sentence(sentenece)
                #添加[cls]和[sep]
                input_ids = [self.tokenizer.cls_token_id] + input_ids + [self.tokenizer.sep_token_id]
                #labels 添加两个8，表示[CLS]和[SEP]对应位置
                labels = [8] + labels + [8]
                labels = self.padding(labels, -1, self.config["max_length"]+2)
    ...
    def padding(self, input_id, pad_token=0, max_length=None):
