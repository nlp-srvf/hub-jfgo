...
from config import LSTM_CONFIG, MODEL_DIR, MAX_SEQ_LENGTH
import os


class LSTMClassifier(nn.Module):
    """LSTM分类模型"""
    def __init__(self, vocab_size, embedding_dim, hidden_dim, num_layers, dropout, bidirectional=True, num_classes=2):
    ...
    def forward(self, x):
        ...
        if self.bidirectional:
            # 拼接前向和后向最后一层的hidden state
            hidden_cat = torch.cat((hidden[-2, :, :], hidden[-1, :, :]), dim=1)
        else:
            hidden_cat = hidden[-1, :, :]
        
        hidden_cat = self.dropout(hidden_cat)
        logits = self.fc(hidden_cat)  # (batch_size, num_classes)
